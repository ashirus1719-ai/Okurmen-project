...
**Статус:** реализовано в миграциях Prisma и применено к Neon
**Дата:** 24 сентября 2026 г.

> Этот документ является логической и физической моделью данных. Реализация разбита на транзакционные миграции в `prisma/migrations/20260924*` и применена к ветке Neon `production` 24 сентября 2026 г. Повторное развёртывание выполняется командой `npm run db:deploy`, проверка — `npm run db:verify`.
